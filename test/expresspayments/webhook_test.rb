...
module ExpressPayments
  class WebhookTest < Test::Unit::TestCase
    ...
    def generate_header(opts = {})
      ...
      opts[:secret] ||= SECRET
      opts[:scheme] ||= ExpressPayments::Webhook::Signature::EXPECTED_SCHEME
      opts[:signature] ||= ExpressPayments::Webhook::Signature.compute_signature(
        opts[:timestamp],
        opts[:payload],
        opts[:secret]
      )
      ExpressPayments::Webhook::Signature.generate_header(
        opts[:timestamp],
        opts[:signature],
        scheme: opts[:scheme]
      )
    ...
    context ".compute_signature" do
      should "compute a signature which can then be verified" do
        timestamp = Time.now
        signature = ExpressPayments::Webhook::Signature.compute_signature(
          timestamp,
          EVENT_PAYLOAD,
          SECRET
        )
        header = generate_header(timestamp: timestamp, signature: signature)
        assert(ExpressPayments::Webhook::Signature.verify_header(EVENT_PAYLOAD, header, SECRET))
      end
    end
    ...
    context ".generate_header" do
      should "generate a header in valid format" do
        ...
        scheme = "v1"
        header = ExpressPayments::Webhook::Signature.generate_header(
          timestamp,
          signature,
          scheme: scheme
        )
    ...
    context ".construct_event" do
      should "return an Event instance from a valid JSON payload and valid signature header" do
        ...
        event = ExpressPayments::Webhook.construct_event(EVENT_PAYLOAD, header, SECRET)
        assert event.is_a?(ExpressPayments::Event)
      end
      ...
      should "raise a JSON::ParserError from an invalid JSON payload" do
        assert_raises JSON::ParserError do
          ...
          ExpressPayments::Webhook.construct_event(payload, header, SECRET)
        end
      end

      should "raise a SignatureVerificationError from a valid JSON payload and an invalid signature header" do
        header = "bad_header"
        assert_raises ExpressPayments::SignatureVerificationError do
          ExpressPayments::Webhook.construct_event(EVENT_PAYLOAD, header, SECRET)
        end
      end
    end
    ...
    context ".verify_signature_header" do
      should "raise a SignatureVerificationError when the header does not have the expected format" do
        ...
        e = assert_raises(ExpressPayments::SignatureVerificationError) do
          ExpressPayments::Webhook::Signature.verify_header(EVENT_PAYLOAD, header, "secret")
        end
      ...
      should "raise a SignatureVerificationError when the timestamp is not within the tolerance" do
        ...
        e = assert_raises(ExpressPayments::SignatureVerificationError) do
          ExpressPayments::Webhook::Signature.verify_header(EVENT_PAYLOAD, header, SECRET, tolerance: 10)
        end
      ...
      should "return true when the header contains a valid signature and the timestamp is within the tolerance" do
        header = generate_header
        assert(ExpressPayments::Webhook::Signature.verify_header(EVENT_PAYLOAD, header, SECRET, tolerance: 10))
      end
