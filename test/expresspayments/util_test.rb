  ...
  class UtilTest < Test::Unit::TestCase
    context "OPTS_COPYABLE" do
      should "include :apibase" do
        assert_include ExpressPayments::Util::OPTS_COPYABLE, :api_base
      end
    end

    context "OPTS_PERSISTABLE" do
      should "include :client" do
        assert_include ExpressPayments::Util::OPTS_PERSISTABLE, :client
      end

      should "not include :idempotency_key" do
        refute_includes ExpressPayments::Util::OPTS_PERSISTABLE, :idempotency_key
      end
    end
    ...
    should "#encode_parameters should prepare parameters for an HTTP request" do
      ...
      assert_equal(
        ...
        ExpressPayments::Util.encode_parameters(params)
      )
    end

    should "#url_encode should prepare strings for HTTP" do
      assert_equal "foo", ExpressPayments::Util.url_encode("foo")
      assert_equal "foo", ExpressPayments::Util.url_encode(:foo)
      assert_equal "foo%2B", ExpressPayments::Util.url_encode("foo+")
      assert_equal "foo%26", ExpressPayments::Util.url_encode("foo&")
      assert_equal "foo[bar]", ExpressPayments::Util.url_encode("foo[bar]")
    end
    ...
    should "#flatten_params should encode parameters according to Rails convention" do
      ...
      assert_equal([
        ...
        ["f[1][foo]", "3"],
        ["f[1][bar]", "4"],
      ], ExpressPayments::Util.flatten_params(params))
    end
    ...
    should "#symbolize_names should convert names to symbols" do
      ...
      symbolized = ExpressPayments::Util.symbolize_names(start)
      assert_equal(finish, symbolized)
    end

    should "#normalize_opts should reject nil keys" do
      assert_raise { ExpressPayments::Util.normalize_opts(nil) }
      assert_raise { ExpressPayments::Util.normalize_opts(api_key: nil) }
    end

    should "#convert_to_expresspayments_object should pass through unknown types" do
      obj = Util.convert_to_expresspayments_object(7, {})
      assert_equal 7, obj
    end

    should "#convert_to_expresspayments_object should turn hashes into ExpressPaymentsObjects" do
      obj = Util.convert_to_expresspayments_object({ foo: "bar" }, {})
      assert obj.is_a?(ExpressPaymentsObject)
      assert_equal "bar", obj.foo
    end

    should "#convert_to_expresspayments_object should turn lists into ListObjects" do
      obj = Util.convert_to_expresspayments_object({ object: "list" }, {})
      assert obj.is_a?(ListObject)
    end

    should "#convert_to_expresspayments_object should marshal hashes with string keys" do
      obj = Util.convert_to_expresspayments_object({ "object" => "account" }, {})
      assert obj.is_a?(Account)
    end

    should "#convert_to_expresspayments_object should marshal other classes" do
      obj = Util.convert_to_expresspayments_object({ object: "account" }, {})
      assert obj.is_a?(Account)
    end

    should "#convert_to_expresspayments_object should marshal arrays" do
      obj = Util.convert_to_expresspayments_object([1, 2, 3], {})
      assert_equal [1, 2, 3], obj
    end

    context ".request_id_dashboard_url" do
      should "generate a livemode URL" do
        assert_equal "https://dashboard.epayments.network/live/logs/request-id",
                     Util.request_id_dashboard_url("request-id", "sk_live_123")
      end

      should "generate a testmode URL" do
        assert_equal "https://dashboard.epayments.network/test/logs/request-id",
    ...
    context ".log_*" do
      setup do
        @old_log_level = ExpressPayments.log_level
        ExpressPayments.log_level = nil
      ...
      teardown do
        ExpressPayments.log_level = @old_log_level
        $stderr = @old_stderr
        $stdout = @old_stdout
      end
      ...
      context ".log_debug" do
        ...
        should "log if level set to debug" do
          ExpressPayments.log_level = ExpressPayments::LEVEL_DEBUG
          Util.log_debug("foo")
          assert_equal "message=foo level=debug \n", $stdout.string
        end

        should "not log if level set to error" do
          ExpressPayments.log_level = ExpressPayments::LEVEL_ERROR
          Util.log_debug("foo")
          assert_equal "", $stdout.string
        end
    ...
    context ".log_internal" do
      should "log in a terminal friendly way" do
        ...
        Util.send(:log_internal, "message", { foo: "bar" },
                  color: :green, level: ExpressPayments::LEVEL_DEBUG, logger: nil, out: out)
      ...
      should "log to a logger if set" do
        ...
        Util.send(:log_internal, "message", { foo: "bar" },
                  color: :green, level: ExpressPayments::LEVEL_DEBUG, logger: logger, out: $stdout)
