...
module ExpressPayments
  class ExpressPaymentsErrorTest < Test::Unit::TestCase
    context "ExpressPaymentsError" do
      context "#initialize" do
        should "initialize error if json_body is set" do
          e = ExpressPaymentsError.new("message", json_body: { error: { code: "some_error" } })
      ...
      context "#idempotent_replayed?" do
        should "initialize from header" do
          e = ExpressPaymentsError.new("message", http_headers: { "idempotent-replayed" => "true" })
          assert_equal true, e.idempotent_replayed?
        end

        should "be 'falsey' by default" do
          e = ExpressPaymentsError.new("message")
      ...
      context "#to_s" do
        should "convert to string" do
          ...
          e = ExpressPaymentsError.new("message", http_status: 200)
          assert_equal "(Status 200) message", e.to_s

          e = ExpressPaymentsError.new("message", http_status: nil, http_body: nil, json_body: nil, http_headers: { "request-id" => "request-id" })
