...
module ExpressPayments
  class ExpressPaymentsClientTest < Test::Unit::TestCase
    context "initializing an ExpressPaymentsClient" do
      should "allow a String to be passed in order to set the api key" do
        assert_equal ExpressPaymentsClient.new("test_123").config.api_key, "test_123"
      end

      should "allow for overrides via a Hash" do
        config = { api_key: "test_123", open_timeout: 100 }
        client = ExpressPaymentsClient.new(config)
      ...
      should "support deprecated ConnectionManager objects" do
        assert ExpressPaymentsClient.new(ExpressPayments::ConnectionManager.new).config.is_a?(ExpressPayments::ExpressPaymentsConfiguration)
      end

      should "support passing a full configuration object" do
        config = ExpressPayments.config.reverse_duplicate_merge({ api_key: "test_123", open_timeout: 100 })
        client = ExpressPaymentsClient.new(config)
    ...
    context ".active_client" do
      should "be .default_client outside of #request" do
        assert_equal ExpressPaymentsClient.default_client, ExpressPaymentsClient.active_client
      end

      should "be active client inside of #request" do
        client = ExpressPaymentsClient.new
        client.request do
          assert_equal client, ExpressPaymentsClient.active_client
        end
      end
    end
    ...
    context ".maybe_gc_connection_managers" do
      should "garbage collect old connection managers when appropriate" do
        stub_request(:post, "#{ExpressPayments.api_base}/v1/path")
          .to_return(body: JSON.generate(object: "account"))

        # Make sure we start with a blank slate (state may have been left in
        # place by other tests).
        ExpressPaymentsClient.clear_all_connection_managers

        # Establish a base time.
        t = 0.0

        # And pretend that `ExpressPaymentsClient` was just initialized for the first
        # time. (Don't access instance variables like this, but it's tricky to
        # test properly otherwise.)
        ExpressPaymentsClient.instance_variable_set(:@last_connection_manager_gc, t)
        ...
        assert_equal nil, ExpressPaymentsClient.maybe_gc_connection_managers

        #
        # t + ExpressPaymentsClient::CONNECTION_MANAGER_GC_PERIOD - 1
        #
        # Move time to just *before* garbage collection is eligible to run.
        # Nothing should happen.
        #
        Util.stubs(:monotonic_time).returns(t + ExpressPaymentsClient::CONNECTION_MANAGER_GC_PERIOD - 1)
        ...
        assert_equal 0, ExpressPaymentsClient.maybe_gc_connection_managers

        #
        # t + ExpressPaymentsClient::CONNECTION_MANAGER_GC_LAST_USED_EXPIRY + 1
        #
        ...
        #
        Util.stubs(:monotonic_time).returns(t + ExpressPaymentsClient::CONNECTION_MANAGER_GC_LAST_USED_EXPIRY + 1)

        assert_equal 1, ExpressPaymentsClient.maybe_gc_connection_managers

        # And as an additional check, the connection manager of the current
        # thread context should have been removed as it was GCed.
        assert_equal({}, ExpressPaymentsClient.current_thread_context.default_connection_managers)
      end
      ...
      should "only garbage collect when all connection managers for a thread are expired" do
        ...
        # connection managers
        active_client = ExpressPaymentsClient.new(max_network_retries: 10)
        active_client.execute_request(:post, "/v1/path")

        assert_equal 2, ExpressPaymentsClient.current_thread_context.default_connection_managers.keys.count
        assert_equal nil, ExpressPaymentsClient.maybe_gc_connection_managers
        ...
        ExpressPaymentsClient.default_connection_manager(active_client.config)
    ...
    context ".clear_all_connection_managers" do
      should "clear connection managers across all threads" do
        ...
        threads = num_threads.times.map do |_|
          Thread.start do
            ...
            manager = ExpressPaymentsClient.default_connection_manager
      ...
      should "clear only connection managers with a given configuration" do
        ...
        client1 = ExpressPaymentsClient.new(read_timeout: 5.0)
        ExpressPaymentsClient.default_connection_manager(client1.config)
        client2 = ExpressPaymentsClient.new(read_timeout: 2.0)
        ExpressPaymentsClient.default_connection_manager(client2.config)

        thread_contexts = ExpressPaymentsClient.instance_variable_get(:@thread_contexts_with_connection_managers)
        ...
        ExpressPaymentsClient.clear_all_connection_managers(config: client1.config)
    ...
    context ".default_client" do
      should "be an ExpressPaymentsClient" do
        assert_kind_of ExpressPaymentsClient, ExpressPaymentsClient.default_client
      end

      should "be a different client on each thread" do
        other_thread_client = nil
        thread = Thread.new do
          other_thread_client = ExpressPaymentsClient.default_client
        end
        thread.join
        refute_equal ExpressPaymentsClient.default_client, other_thread_client
      end
    end
    ...
    context ".default_connection_manager" do
      should "be a ConnectionManager" do
        assert_kind_of ConnectionManager,
                       ExpressPaymentsClient.default_connection_manager
      end

      should "be a different connection on each thread" do
        other_thread_manager = nil
        thread = Thread.new do
          other_thread_manager = ExpressPaymentsClient.default_connection_manager
        end
        thread.join
        refute_equal ExpressPaymentsClient.default_connection_manager, other_thread_manager
      end

      should "create a separate connection manager per configuration" do
        config = ExpressPayments::ExpressPaymentsConfiguration.setup { |c| c.open_timeout = 100 }
        connection_manager_one = ExpressPaymentsClient.default_connection_manager
        connection_manager_two = ExpressPaymentsClient.default_connection_manager(config)
      ...
      should "create a single connection manager for identical configurations" do
        ...
        2.times { ExpressPaymentsClient.default_connection_manager(ExpressPayments::ExpressPaymentsConfiguration.setup) }

        assert_equal 1, ExpressPaymentsClient.instance_variable_get(:@thread_contexts_with_connection_managers).first.default_connection_managers.size
      end
    end

    context ".should_retry?" do
      setup do
        ExpressPayments::ExpressPaymentsConfiguration.any_instance.stubs(:max_network_retries).returns(2)
      end

      should "retry on Errno::ECONNREFUSED" do
        assert ExpressPaymentsClient.should_retry?(Errno::ECONNREFUSED.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on EOFError" do
        assert ExpressPaymentsClient.should_retry?(EOFError.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on Errno::ECONNRESET" do
        assert ExpressPaymentsClient.should_retry?(Errno::ECONNRESET.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on Errno::ETIMEDOUT" do
        assert ExpressPaymentsClient.should_retry?(Errno::ETIMEDOUT.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on Errno::EHOSTUNREACH" do
        assert ExpressPaymentsClient.should_retry?(Errno::EHOSTUNREACH.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on Net::OpenTimeout" do
        assert ExpressPaymentsClient.should_retry?(Net::OpenTimeout.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on Net::ReadTimeout" do
        assert ExpressPaymentsClient.should_retry?(Net::ReadTimeout.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry on SocketError" do
        assert ExpressPaymentsClient.should_retry?(SocketError.new,
                                                   method: :post, num_retries: 0)
      end

      should "retry when the `EP-Should-Retry` header is `true`" do
        headers = ExpressPaymentsResponse::Headers.new(
          "EP-Should-Retry" => ["true"]
        )

        # Note we send status 400 here, which would normally not be retried.
        assert ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_headers: headers,
                                                                                             http_status: 400),
                                                   method: :post, num_retries: 0)
      end

      should "not retry when the `EP-Should-Retry` header is `false`" do
        headers = ExpressPaymentsResponse::Headers.new(
          "EP-Should-Retry" => ["false"]
        )

        # Note we send status 409 here, which would normally be retried.
        refute ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_headers: headers,
                                                                                             http_status: 409),
                                                   method: :post, num_retries: 0)
      end

      should "retry on a 409 Conflict" do
        assert ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 409),
                                                   method: :post, num_retries: 0)
      end

      should "retry on a 429 Too Many Requests when lock timeout" do
        assert ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 429,
                                                                                             code: "lock_timeout"),
                                                   method: :post, num_retries: 0)
      end

      should "retry on a 500 Internal Server Error when non-POST" do
        assert ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 500),
                                                   method: :get, num_retries: 0)
      end

      should "retry on a 503 Service Unavailable" do
        assert ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 503),
                                                   method: :post, num_retries: 0)
      end

      should "not retry at maximum count" do
        refute ExpressPaymentsClient.should_retry?(RuntimeError.new,
                                                   method: :post, num_retries: ExpressPayments.max_network_retries)
      end

      should "not retry on a certificate validation error" do
        refute ExpressPaymentsClient.should_retry?(OpenSSL::SSL::SSLError.new,
                                                   method: :post, num_retries: 0)
      end

      should "not retry on a 429 Too Many Requests when not lock timeout" do
        refute ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 429,
                                                                                             code: "rate_limited"),
                                                   method: :post, num_retries: 0)
      end

      should "not retry on a 500 Internal Server Error when POST" do
        refute ExpressPaymentsClient.should_retry?(ExpressPayments::ExpressPaymentsError.new(http_status: 500),
                                                   method: :post, num_retries: 0)
      end
    end
    ...
    context ".sleep_time" do
      should "should grow exponentially" do
        ExpressPaymentsClient.stubs(:rand).returns(1)
        ExpressPayments.config.stubs(:max_network_retry_delay).returns(999)
        assert_equal(ExpressPayments.initial_network_retry_delay, ExpressPaymentsClient.sleep_time(1))
        assert_equal(ExpressPayments.initial_network_retry_delay * 2, ExpressPaymentsClient.sleep_time(2))
        assert_equal(ExpressPayments.initial_network_retry_delay * 4, ExpressPaymentsClient.sleep_time(3))
        assert_equal(ExpressPayments.initial_network_retry_delay * 8, ExpressPaymentsClient.sleep_time(4))
      end

      should "enforce the max_network_retry_delay" do
        ExpressPaymentsClient.stubs(:rand).returns(1)
        ExpressPayments.config.stubs(:initial_network_retry_delay).returns(1)
        ExpressPayments.config.stubs(:max_network_retry_delay).returns(2)
        assert_equal(1, ExpressPaymentsClient.sleep_time(1))
        assert_equal(2, ExpressPaymentsClient.sleep_time(2))
        assert_equal(2, ExpressPaymentsClient.sleep_time(3))
        assert_equal(2, ExpressPaymentsClient.sleep_time(4))
      end

      should "add some randomness" do
        random_value = 0.8
        ExpressPaymentsClient.stubs(:rand).returns(random_value)
        ExpressPayments.config.stubs(:initial_network_retry_delay).returns(1)
        ExpressPayments.config.stubs(:max_network_retry_delay).returns(8)

        base_value = ExpressPayments.initial_network_retry_delay * (0.5 * (1 + random_value))
        ...
        assert_equal(base_value * 2, ExpressPaymentsClient.sleep_time(2))
        assert_equal(base_value * 4, ExpressPaymentsClient.sleep_time(3))
        assert_equal(base_value * 8, ExpressPaymentsClient.sleep_time(4))
      end
      ...
      should "permit passing in a configuration object" do
        ...
        ExpressPayments.config.stubs(:initial_network_retry_delay).returns(100)
        ExpressPayments.config.stubs(:max_network_retry_delay).returns(200)

        assert_equal(1, ExpressPaymentsClient.sleep_time(1, config: config))
        assert_equal(2, ExpressPaymentsClient.sleep_time(2, config: config))
        assert_equal(2, ExpressPaymentsClient.sleep_time(3, config: config))
        assert_equal(2, ExpressPaymentsClient.sleep_time(4, config: config))
      end
    end
      ...
      context "request processing for #{request_method}" do
        ...
        context "headers" do
          should "support literal headers" do
            stub_request(:post, "#{ExpressPayments.api_base}/v1/account")
              .with(headers: { "EP-Account" => "bar" })
              .to_return(body: JSON.generate(object: "account"))
        ...
        context "logging" do
          setup do
            ...
            # Stub the ExpressPayments.config so that mocha matchers will succeed. Currently,
            # mocha does not support using param matchers within hashes.
            ExpressPaymentsClient.any_instance.stubs(:config).returns(ExpressPayments.config)
          end
          ...
          should "produce appropriate logging" do
            ...
            Util.expects(:log_info).with("Request to ExpressPayments API",
                                         account: "acct_123",
            ...
            Util.expects(:log_info).with("Response from ExpressPayments API",
                                         account: "acct_123",
                                         api_version: "2023-11-01",
                                         elapsed: 0.0,
                                         idempotency_key: "abc",
            ...
            Util.expects(:log_debug).with do |message, data|
              if message == "Response details" &&
                 ...
                 data[:config] == ExpressPayments.config &&
                 data[:process_id] == Process.pid &&
                 data[:thread_object_id] == Thread.current.object_id &&
            ...
            Util.expects(:log_debug).with("Dashboard link for request",
                                          idempotency_key: "abc",
                                          request_id: "req_123",
                                          url: Util.request_id_dashboard_url("req_123", ExpressPayments.api_key),
                                          config: ExpressPayments.config)
          ...
          should "produce logging on API error" do
            ...
            Util.expects(:log_error).with("ExpressPayments API error",
                                          status: 500,
            ...
            client = ExpressPaymentsClient.new
            assert_raises ExpressPayments::APIError do
              client.send(request_method, :post, "/v1/account",
                          &@read_body_chunk_block)
            end
          end
          ...
          should "produce logging on OAuth error" do
            ...
            Util.expects(:log_error).with("ExpressPayments OAuth error",
                                          status: 400,
            ...
            client = ExpressPaymentsClient.new
            opts = { api_base: ExpressPayments.connect_base }
            assert_raises ExpressPayments::OAuth::InvalidRequestError do
        ...
        context "EP-Account header" do
          should "use a globally set header" do
            begin
              old = ExpressPayments.ep_account
              ExpressPayments.ep_account = "acct_1234"

              stub_request(:post, "#{ExpressPayments.api_base}/v1/account")
                .with(headers: { "EP-Account" => ExpressPayments.ep_account })
                .to_return(body: JSON.generate(object: "account"))
              ...
              ExpressPayments.ep_account = old
            end
          end

          should "use a locally set header" do
            ep_account = "acct_0000"
            stub_request(:post, "#{ExpressPayments.api_base}/v1/account")
              .with(headers: { "EP-Account" => ep_account })
              .to_return(body: JSON.generate(object: "account"))

            client = ExpressPaymentsClient.new
            client.send(request_method, :post, "/v1/account",
                        headers: { ep_account: ep_account },
                        &@read_body_chunk_block)
          end
          ...
          should "send app_info if set" do
            begin
              ...
              stub_request(:post, "#{ExpressPayments.api_base}/v1/account")
                .with do |req|
                  assert_equal \
                    "ExpressPayments/v1 RubyBindings/#{ExpressPayments::VERSION} " \
                    "MyAwesomePlugin/1.2.34 (https://myawesomeplugin.info)",
                    req.headers["User-Agent"]

                  data = JSON.parse(req.headers["X-EP-Client-User-Agent"],
        ...
        context "error handling" do
          should "handle error response with empty body" do
            stub_request(:post, "#{ExpressPayments.api_base}/v1/charges")
              .to_return(body: "", status: 500)

            client = ExpressPaymentsClient.new

            e = assert_raises ExpressPayments::APIError do
          ...
          should "feed a request ID through to the error object" do
            ...
            e = assert_raises ExpressPayments::InvalidRequestError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "handle low level error" do
            ...
            e = assert_raises ExpressPayments::APIConnectionError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
            assert_equal (ExpressPaymentsClient::ERROR_MESSAGE_CONNECTION % ExpressPayments.api_base) +
          ...
          should "raise IdempotencyError on 400 of type idempotency_error" do
            ...
            e = assert_raises ExpressPayments::IdempotencyError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "raise AuthenticationError on 401" do
            ...
            e = assert_raises ExpressPayments::AuthenticationError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "raise CardError on 402" do
            ...
            e = assert_raises ExpressPayments::CardError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "raise PermissionError on 403" do
            ...
            e = assert_raises ExpressPayments::PermissionError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "raise RateLimitError on 429" do
            ...
            e = assert_raises ExpressPayments::RateLimitError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "raise OAuth::InvalidRequestError when error is a string with value 'invalid_request'" do
            ...
            opts = { api_base: ExpressPayments.connect_base }
            e = assert_raises ExpressPayments::OAuth::InvalidRequestError do
              client.send(request_method, :post, "/oauth/token", **opts,
                          &@read_body_chunk_block)
            end
          ...
          should "raise OAuth::InvalidGrantError when error is a string with value 'invalid_grant'" do
            ...
            opts = { api_base: ExpressPayments.connect_base }
            e = assert_raises ExpressPayments::OAuth::InvalidGrantError do
              client.send(request_method, :post, "/oauth/token", **opts,
                          &@read_body_chunk_block)
            end
          ...
          should "raise OAuth::InvalidClientError when error is a string with value 'invalid_client'" do
            stub_request(:post, "#{ExpressPayments.connect_base}/oauth/deauthorize")
              .to_return(body: JSON.generate(error: "invalid_client",
                                             error_description: "This application is not connected to expresspayments account acct_19tLK7DSlTMT26Mk, or that account does not exist."), status: 401)

            client = ExpressPaymentsClient.new

            opts = { api_base: ExpressPayments.connect_base }
            e = assert_raises ExpressPayments::OAuth::InvalidClientError do
            ...
            assert_equal("This application is not connected to expresspayments account acct_19tLK7DSlTMT26Mk, or that account does not exist.", e.message)
          end

          should "raise ExpressPayments::OAuthError on indeterminate OAuth error" do
            stub_request(:post, "#{ExpressPayments.connect_base}/oauth/deauthorize")
            ...
            opts = { api_base: ExpressPayments.connect_base }
            e = assert_raises ExpressPayments::OAuth::OAuthError do
              client.send(request_method, :post, "/oauth/deauthorize", **opts,
                          &@read_body_chunk_block)
            end
        ...
        context "idempotency keys" do
          ...
          should "not add an idempotency key to GET requests" do
            ...
            stub_request(:get, "#{ExpressPayments.api_base}/v1/charges/ch_123")
              .with do |req|
          ...
          should "ensure there is always an idempotency_key on DELETE requests" do
            SecureRandom.expects(:uuid).at_least_once.returns("random_key")
            stub_request(:delete, "#{ExpressPayments.api_base}/v1/charges/ch_123")
              .with(headers: { "Idempotency-Key" => "random_key" })
              .to_return(body: JSON.generate(object: "charge"))
            client = ExpressPaymentsClient.new
        ...
        context "retry logic" do
          ...
          should "retry failed requests and raise if error persists" do
            ExpressPaymentsClient.expects(:sleep_time).at_least_once.returns(0)
            stub_request(:post, "#{ExpressPayments.api_base}/v1/charges")
              .to_raise(Errno::ECONNREFUSED.new)

            client = ExpressPaymentsClient.new
            err = assert_raises ExpressPayments::APIConnectionError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          ...
          should "pass the client configuration when retrying" do
            ExpressPaymentsClient.expects(:sleep_time)
                        .with(any_of(1, 2),
                              has_entry(:config, kind_of(ExpressPayments::ExpressPaymentsConfiguration)))
            ...
            client = ExpressPaymentsClient.new
            assert_raises ExpressPayments::APIConnectionError do
              client.send(request_method, :post, "/v1/charges",
                          &@read_body_chunk_block)
            end
          end
        ...
        context "params serialization" do
          should "allows empty strings in params" do
            ...
            assert_requested(
              :get,
              "#{ExpressPayments.api_base}/v1/invoices/upcoming?",
              query: {
                customer: "cus_123",
                coupon: "",
              }
    ...
    context "#execute_request_stream" do
      ...
      should "executes the read_body_chunk_block when passed" do
        ...
        assert_instance_of ExpressPayments::ExpressPaymentsHeadersOnlyResponse, resp
    ...
    context "#connection_manager" do
      should "warn that #connection_manager is deprecated" do
        ...
        begin
          ...
          message = "NOTE: ExpressPayments::ExpressPaymentsClient#connection_manager is " \
                    "deprecated"
    ...
    context "#request" do
      should "return a result and response object" do
        ...
        assert resp.is_a?(ExpressPaymentsResponse)
      ...
      should "reset local thread state after a call" do
        begin
          ExpressPaymentsClient.current_thread_context.active_client = :expresspayments_client

          client = ExpressPaymentsClient.new
          client.request {} # rubocop:todo Lint/EmptyBlock

          assert_equal :expresspayments_client,
                       ExpressPaymentsClient.current_thread_context.active_client
        ensure
          ExpressPaymentsClient.current_thread_context.active_client = nil
        end
      end
      ...
      should "correctly return last responses despite multiple clients" do
        ...
        stub_request(:post, "#{ExpressPayments.api_base}/v1/coupons")
          .to_return(body: JSON.generate(coupon_resp))

        client1 = ExpressPaymentsClient.new
        client2 = ExpressPaymentsClient.new
      ...
      should "error if calls to #request are nested on the same thread" do
        ...
        client.request do
          ...
          assert_equal "calls to ExpressPaymentsClient#request cannot be nested within a thread",
                       e.message
        end
      end
    end
    ...
    context "#proxy" do
      should "run the request through the proxy" do
        begin
          ...
          connection = ExpressPayments::ExpressPaymentsClient.default_connection_manager.connection_for(ExpressPayments.api_base)
    ...
    context "#telemetry" do
      ...
      should "not send metrics if enable trace flag is not set" do
        ...
        stub_request(:get, "#{ExpressPayments.api_base}/v1/charges")
          .with do |req|
          trace_metrics_header = req.headers["X-EP-Client-Telemetry"]
          false
        end.to_return(body: JSON.generate(object: "charge"))

        ExpressPayments::Charge.list
      ...
      should "send metrics if enabled telemetry is true" do
        ExpressPayments.enable_telemetry = true
    ...
    context "instrumentation" do
      teardown do
        ExpressPayments::Instrumentation.unsubscribe(:request, :test)
      end

      should "notify a subscribe on HTTP request start" do
        events = []
        ExpressPayments::Instrumentation.subscribe(:request_end, :test) { |event| events << event }
      ...
      should "notify a subscriber of a successful HTTP request" do
        ...
        ExpressPayments::Charge.create(amount: 50, currency: "usd", card: "sc_token")
      ...
      should "notify a subscriber of an ExpressPaymentsError" do
        events = []
        ExpressPayments::Instrumentation.subscribe(:request_end, :test) { |event| events << event }
      ...
      should "pass `user_data` from `request_begin` to `request_end`" do
        ...
        ExpressPayments::Instrumentation.subscribe(:request_begin) do |event|
          event.user_data[:foo] = :bar
        end
        ExpressPayments::Instrumentation.subscribe(:request_end) do |event|
      ...
      should "provide backward compatibility on `request` topic" do
        events = []
        ExpressPayments::Instrumentation.subscribe(:request, :test) { |event| events << event }
  ...
  class SystemProfilerTest < Test::Unit::TestCase
    context "#uname" do
      should "run without failure" do
        ...
        _ = ExpressPaymentsClient::SystemProfiler.uname
      end
    end
    ...
    context "#uname_from_system" do
      should "run without failure" do
        ...
        _ = ExpressPaymentsClient::SystemProfiler.uname_from_system
      end
    end
    ...
    context "#uname_from_system_ver" do
      should "run without failure" do
        # as above, just verify that an exception is not thrown
        _ = ExpressPaymentsClient::SystemProfiler.uname_from_system_ver
      end
    end
  end
end
