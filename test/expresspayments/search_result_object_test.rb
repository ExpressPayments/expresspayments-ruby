...
module ExpressPayments
  class SearchResultObjectTest < Test::Unit::TestCase
    should "provide .empty_list" do
      list = ExpressPayments::SearchResultObject.empty_search_result
      assert list.empty?
    end

    should "provide #count via enumerable" do
      list = ExpressPayments::SearchResultObject.construct_from(data: [{ object: "charge" }])
    ...
    should "provide #each" do
      ...
      expected = Util.convert_to_expresspayments_object(arr, {})
      list = ExpressPayments::SearchResultObject.construct_from(data: arr)
    ...
    should "provide #auto_paging_each that supports forward pagination" do
      ...
      # iteration stops.
      stub_request(:get, "#{ExpressPayments.api_base}/things")
    ...
    should "provide #empty?" do
      list = ExpressPayments::SearchResultObject.construct_from(data: [])
      assert list.empty?
      list = ExpressPayments::SearchResultObject.construct_from(data: [{}])
      refute list.empty?
    end
    ...
    should "fetch an empty page through #next_page" do
      list = TestSearchResultObject.construct_from(data: [{ id: 1 }],
                                                   ...
                                                   url: "/things")
      next_list = list.next_search_result_page
      assert_equal ExpressPayments::SearchResultObject.empty_search_result, next_list
    end
  end
end

# A helper class with a URL that allows us to try out pagination.
class TestSearchResultObject < ExpressPayments::SearchResultObject
end
