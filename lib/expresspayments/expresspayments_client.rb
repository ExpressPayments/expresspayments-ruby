...
require "expresspayments/instrumentation"

module ExpressPayments
  # ExpressPaymentsClient executes requests against the ExpressPayments API and allows a user to
  # recover both a resource a call returns as well as a response object that
  # contains information on the HTTP call.
  class ExpressPaymentsClient
    ...
    # Initializes a new ExpressPaymentsClient
    def initialize(config_arg = {})
      ...
      @config = case config_arg
                when Hash
                  ExpressPayments.config.reverse_duplicate_merge(config_arg)
                when ExpressPayments::ConnectionManager
                  # Supports accepting a connection manager object for backwards
                  # compatibility only, and that use is DEPRECATED.
                  ExpressPayments.config.dup
                when ExpressPayments::ExpressPaymentsConfiguration
                  config_arg
                when String
                  ExpressPayments.config.reverse_duplicate_merge(
                    { api_key: config_arg }
                  )
                else
    ...
    # Gets a currently active `ExpressPaymentsClient`. Set for the current thread when
    # `ExpressPaymentsClient#request` is being run so that API operations being executed
    ...
    def self.default_client
      current_thread_context.default_client ||= ExpressPaymentsClient.new(ExpressPayments.config)
    end

    # A default connection manager for the current thread scoped to the
    # configuration object that may be provided.
    def self.default_connection_manager(config = ExpressPayments.config)
    ...
    def self.should_retry?(error,
                           method:, num_retries:, config: ExpressPayments.config)
      ...
      when EOFError, Errno::ECONNREFUSED, Errno::ECONNRESET, # rubocop:todo Lint/DuplicateBranch
        ...
        true
      when ExpressPayments::ExpressPaymentsError
        # The API may ask us not to retry (e.g. if doing so would be a no-op),
        # or advise us to retry (e.g. in cases of lock timeouts). Defer to
        # those instructions if given.
        return false if error.http_headers["expresspayments-should-retry"] == "false"
        return true if error.http_headers["expresspayments-should-retry"] == "true"
    ...
    def self.sleep_time(num_retries, config: ExpressPayments.config)
    ...
    # Gets the connection manager in use for the current `ExpressPaymentsClient`.
    #
    ...
    #
    def request
      old_expresspayments_client = self.class.current_thread_context.active_client
      self.class.current_thread_context.active_client = self

      if self.class.current_thread_context.last_responses&.key?(object_id)
        raise "calls to ExpressPaymentsClient#request cannot be nested within a thread"
      end
      ...
      begin
        ...
        self.class.current_thread_context.active_client = old_expresspayments_client
    ...
    def execute_request(method, path,
      ...
      # If being called from `ExpressPaymentsClient#request`, put the last response in
    ...
    # it to an ExpressPaymentsObject. This should be used for any request where we expect
    ...
    #
    # If a block is passed, an ExpressPaymentsHeadersOnlyResponse is returned as the
    # block is expected to do all the necessary body processing. If no block is
    # passed, then an ExpressPaymentsStreamResponse is returned containing an IO stream
    ...
    def execute_request_stream(method, path,
      ...
      resp = ExpressPaymentsHeadersOnlyResponse.from_net_http(http_resp)
    ...
    ERROR_MESSAGE_CONNECTION =
      ...
      "ExpressPayments (%s). You may be seeing this message because your DNS is not " \
      "working or you don't have an internet connection.  To check, try " \
      "running `host epayments.network` from the command line."
    ERROR_MESSAGE_SSL =
      "Could not establish a secure connection to ExpressPayments (%s), you " \
      "may need to upgrade your OpenSSL version. To check, try running " \
      "`openssl s_client -connect api.epayments.network:443` from the command " \
      "line."

    # Common error suffix sared by both connect and read timeout messages.
    ERROR_MESSAGE_TIMEOUT_SUFFIX =
      "Please check your internet connection and try again. " \
      "If this problem persists, you should check ExpressPayments' service " \
      "status at https://status.epayments.network, or let us know at " \
      "support@epayments.network."

    ERROR_MESSAGE_TIMEOUT_CONNECT = (
      "Timed out connecting to ExpressPayments (%s). " +
      ERROR_MESSAGE_TIMEOUT_SUFFIX
    ).freeze

    ERROR_MESSAGE_TIMEOUT_READ = (
      "Timed out communicating with ExpressPayments (%s). " +
      ERROR_MESSAGE_TIMEOUT_SUFFIX
    ).freeze
    ...
    # A record representing any data that `ExpressPaymentsClient` puts into
    ...
    # with future non-major changes.
    class ThreadContext
      # A `ExpressPaymentsClient` that's been flagged as currently active within a
      # thread by `ExpressPaymentsClient#request`. A client stays active until the
      # completion of the request block.
      attr_accessor :active_client

      # A default `ExpressPaymentsClient` object for the thread. Used in all cases where
      # the user hasn't specified their own.
      attr_accessor :default_client

      # A temporary map of object IDs to responses from last executed API
      # calls. Used to return a responses from calls to `ExpressPaymentsClient#request`.
      #
      # Stored in the thread data to make the use of a single `ExpressPaymentsClient`
      # object safe across multiple threads. Stored as a map so that multiple
      # `ExpressPaymentsClient` objects can run concurrently on the same thread.
      #
      # Responses are only left in as long as they're needed, which means
      # they're removed as soon as a call leaves `ExpressPaymentsClient#request`, and
      ...
      # all `ExpressPaymentsClient` objects on a particular thread, and created so as to
    ...
    # Access data stored for `ExpressPaymentsClient` within the thread's current
    # context. Returns `ThreadContext`.
    #
    # For internal use only. Does not provide a stable API and may be broken
    # with future non-major changes.
    def self.current_thread_context
      Thread.current[:expresspayments_client__internal_use_only] ||= ThreadContext.new
    end
    ...
    private def execute_request_internal(method, path,
      ...
      context.account         = headers["EP-Account"]
      context.api_key         = api_key
      context.api_version     = headers["EP-Version"]
    ...
    private def check_api_key!(api_key)
      unless api_key
        raise AuthenticationError, "No API key provided. " \
                                   'Set your API key using "ExpressPayments.api_key = <API-KEY>". ' \
                                   "You can generate API keys from the ExpressPayments web interface. " \
                                   "See https://epayments.network/api for details, or email " \
                                   "support@epayments.network if you have any questions."
      end
      ...
      raise AuthenticationError, "Your API key is invalid, as it contains " \
                                 ...
                                 "ExpressPayments web interface. See https://epayments.network/api for details, or " \
                                 "email support@epayments.network if you have any questions.)"
    end
    ...
    private def execute_request_with_rescues(method, api_base, headers, context)
      ...
      begin
        ...
        if config.enable_telemetry? && context.request_id
          ...
          @last_request_metrics =
            ExpressPaymentsRequestMetrics.new(context.request_id, request_duration_ms)
        end
      ...
      # type of exception that we didn't expect to handle.
        ...
        if e.is_a?(ExpressPayments::ExpressPaymentsError)
    ...
    private def notify_request_begin(context)
      ...
      ExpressPayments::Instrumentation.notify(:request_begin, event)
    ...
    private def notify_request_end(context, duration, http_status, num_retries,
      ...
      ExpressPayments::Instrumentation.notify(:request_end, event)

      # The name before `request_begin` was also added. Provided for backwards
      # compatibility.
      ExpressPayments::Instrumentation.notify(:request, event)
    end
    ...
    private def handle_error_response(http_resp, context)
      begin
        ...
        raise ExpressPaymentsError, "Indeterminate error" unless error_data
      rescue JSON::ParserError, ExpressPaymentsError
        raise general_api_error(http_resp.code.to_i, http_resp.body)
      end
    ...
    private def specific_api_error(resp, error_data, context)
      Util.log_error("ExpressPayments API error",
    ...
    private def specific_oauth_error(resp, error_code, context)
      ...
      Util.log_error("ExpressPayments OAuth error",
                     status: resp.http_status,
    ...
    private def handle_network_error(error, context, num_retries,
                                     api_base = nil)
      Util.log_error("ExpressPayments network error",
      ...
      if errors.nil?
        message = "Unexpected error #{error.class.name} communicating " \
                  "with ExpressPayments. Please let us know at support@epayments.network."
      end
    ...
    private def request_headers(api_key, method)
      user_agent = "ExpressPayments/v1 RubyBindings/#{ExpressPayments::VERSION}"
      user_agent += " " + format_app_info(ExpressPayments.app_info) unless ExpressPayments.app_info.nil?
      ...
      if config.enable_telemetry? && !@last_request_metrics.nil?
        headers["X-EP-Client-Telemetry"] = JSON.generate(
      ...
      headers["EP-Version"] = config.api_version if config.api_version
      headers["EP-Account"] = config.ep_account if config.ep_account

      user_agent = @system_profiler.user_agent
      begin
        headers.update(
          "X-EP-Client-User-Agent" => JSON.generate(user_agent)
        )
      rescue StandardError => e
        headers.update(
          "X-EP-Client-Raw-User-Agent" => user_agent.inspect,
          :error => "#{e} (#{e.class})"
        )
      end
    ...
    private def log_request(context, num_retries)
      Util.log_info("Request to ExpressPayments API",
    ...
    private def log_response(context, request_start, status, body, resp)
      Util.log_info("Response from ExpressPayments API",
                    account: context.account,
                    api_version: context.api_version,
    ...
    class RequestLogContext
      ...
      # a `EP-Version` header beyond what configuration information that we
    ...
    class SystemProfiler
      ...
      def user_agent
        ...
        {
          application: ExpressPayments.app_info,
    ...
    # ExpressPaymentsRequestMetrics tracks metadata to be reported to expresspayments for metrics
    # collection
    class ExpressPaymentsRequestMetrics
      # The ExpressPayments request ID of the response.
