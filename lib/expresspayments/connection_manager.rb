...
module ExpressPayments
  ...
  class ConnectionManager
    ...
    # by `ExpressPaymentsClient` to determine whether a connection manager should be
    # garbage collected or not.
    attr_reader :last_used
    attr_reader :config

    def initialize(config = ExpressPayments.config)
      @config = config
      @active_connections = {}
    ...
    private def warn_ssl_verify_none
      ...
      warn("WARNING: Running without SSL cert verification. " \
           ...
           "Execute `ExpressPayments.verify_ssl_certs = true` to enable " \
           "verification.")
    end
  end
end
