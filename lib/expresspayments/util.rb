...
module ExpressPayments
  module Util
    ...
    # Options that should be copyable from one ExpressPaymentsObject to another
    ...
    def self.object_classes
      @object_classes ||= ExpressPayments::ObjectTypes.object_names_to_classes
    end
    ...
    def self.custom_method(resource, target, name, http_verb, http_path)
      ...
      target.define_singleton_method(name) do |id, params = {}, opts = {}|
        ...
        Util.convert_to_expresspayments_object_with_params(resp.data, params, opts)
      end
    end

    # Converts a hash of fields or an array of hashes into a +ExpressPaymentsObject+ or
    # array of +ExpressPaymentsObject+s. These new objects will be created as a concrete
    # type as dictated by their `object` field (e.g. an `object` value of
    # `charge` would create an instance of +Charge+), but if `object` is not
    # present or of an unknown type, the newly created instance will fall back
    # to being a +ExpressPaymentsObject+.
    #
    # ==== Attributes
    #
    # * +data+ - Hash of fields and values to be converted into an ExpressPaymentsObject.
    # * +params+ - Params for +ExpressPaymentsObject+ like filters used in search that
    #   will be reused on subsequent API calls.
    # * +opts+ - Options for +ExpressPaymentsObject+ like an API key that will be reused
    #   on subsequent API calls.
    def self.convert_to_expresspayments_object(data, opts = {})
      convert_to_expresspayments_object_with_params(data, {}, opts)
    end
    ...
    # * +opts+ - Options for +ExpressPaymentsObject+ like an API key that will be reused
    #   on subsequent API calls.
    def self.convert_to_expresspayments_object_with_params(data, params, opts = {})
      opts = normalize_opts(opts)

      case data
      when Array
        data.map { |i| convert_to_expresspayments_object(i, opts) }
      when Hash
        # Try converting to a known object class.  If none available, fall back
        # to generic ExpressPaymentsObject
        object_name = data[:object] || data["object"]
        obj = object_classes.fetch(object_name, ExpressPaymentsObject)
    ...
    def self.log_error(message, data = {})
      config = data.delete(:config) || ExpressPayments.config
      logger = config.logger || ExpressPayments.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= ExpressPayments::LEVEL_ERROR)
        log_internal(message, data, color: :cyan, level: ExpressPayments::LEVEL_ERROR,
                     logger: ExpressPayments.logger, out: $stderr)
      end
    end
    ...
    def self.log_info(message, data = {})
      ...
      logger = config.logger || ExpressPayments.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= ExpressPayments::LEVEL_INFO)
        log_internal(message, data, color: :cyan, level: ExpressPayments::LEVEL_INFO,
                     logger: ExpressPayments.logger, out: $stdout)
      end
    end
    ...
    def self.log_debug(message, data = {})
      ...
      logger = config.logger || ExpressPayments.logger
      if !logger.nil? ||
         (!config.log_level.nil? && config.log_level <= ExpressPayments::LEVEL_DEBUG)
        log_internal(message, data, color: :blue, level: ExpressPayments::LEVEL_DEBUG,
                     logger: ExpressPayments.logger, out: $stdout)
      end
    end
    ...
    def self.request_id_dashboard_url(request_id, api_key)
      ...
      "https://dashboard.epayments.network/#{env}/logs/#{request_id}"
    end
