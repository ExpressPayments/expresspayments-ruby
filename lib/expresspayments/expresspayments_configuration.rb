...
module ExpressPayments
  ...
  class ExpressPaymentsConfiguration
    attr_accessor :api_key, :api_version, :client_id, :enable_telemetry, :logger, :ep_account
    ...
    def initialize
      ...
      @ca_bundle_path = ExpressPayments::DEFAULT_CA_BUNDLE_PATH
      ...
      @api_base = "https://api.epayments.network"
      @connect_base = "https://connect.epayments.network"
      @uploads_base = "https://files.epayments.network"
    end

    def log_level=(val)
      # Backwards compatibility for values that we briefly allowed
      if val == "debug"
        val = ExpressPayments::LEVEL_DEBUG
      elsif val == "info"
        val = ExpressPayments::LEVEL_INFO
      elsif val == "error"
        val = ExpressPayments::LEVEL_ERROR
      end

      levels = [ExpressPayments::LEVEL_INFO, ExpressPayments::LEVEL_DEBUG, ExpressPayments::LEVEL_ERROR]
    ...
    def open_timeout=(open_timeout)
      ...
      ExpressPaymentsClient.clear_all_connection_managers(config: self)
    end
    ...
    # `ExpressPayments.ca_store`) in their initialization code because it marshals lazily
